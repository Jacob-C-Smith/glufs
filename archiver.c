...
int print_directory_structure( char _path[FILENAME_MAX] )
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if ( p_directories[i]->d_type == DT_DIR )
        {
...
int print_node ( glu_file *p_glu_file )
{
    ...
    p_glu_file->osz = load_file(p_glu_file->_name, 0, false);
...
int serializer_node ( FILE *p_file, binary_tree_node *p_binary_tree_node )
{
    ...
    // Write the path
    fwrite(&p_glu_file->_name, file_path_length, 1, p_file);
    
    // Pad the tree entry
    for (size_t i = (239 - file_path_length); i != 0; i--) putc('\0', p_file);
